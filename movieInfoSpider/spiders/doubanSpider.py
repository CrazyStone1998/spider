...
class doubanSpider(scrapy.Spider):
    ...
    cookies = {'push_noty_num': '0', 'ap_v': '0,6.0', 'push_doumail_num': '0',
               '__yadk_uid': 'aWtWeFSSFZ4mNHTL0lkdlQ1JyNZhWoZj', '_pk_ses.100001.8cb4': '*',
               '_pk_ref.100001.8cb4': '%5B%22%22%2C%22%22%2C1578493990%2C%22https%3A%2F%2Faccounts.douban.com%2Fpassport%2Flogin%3Fsource%3Dmovie%22%5D',
               'ck': '0p3c', '_pk_id.100001.8cb4': 'bdfeb174e37cded3.1578493990.1.1578493990.1578493990.',
               'dbcl2': '"188523178:gQGp1cxU65A"', 'bid': 'oyUI6c_33FQ'}
    ...
    def parse_movie_index_douban(self, response):
        ...
        print('name          :', movie.get('title'))
        print('area          :', area)
        print('language      :', language)
        print('length        :', length)
        print('cover_url     :', movie.get('cover'))
        print('rate          :', movie.get('rate'))
        print('release_data  :', release_date)
        print('rating_people :', rating_people)
        print('url_imdb      :', url_imdb)
        print('url_douabn    :', movie.get('url'))
    ...
    def parse_movie_comment_douban(self, response):
        ...
        #     name = each.xpath('.//*[@class="avatar"]/a/@title').extract_first()
        #     url_douban = each.xpath('//*[@class="avatar"]/a/@href').extract_first()
        #     icon = each.xpath('.//*[@class="avatar"]/a/img/@src').extract_first()
        #     votes = each.xpath('.//*[@class="votes"]/text()').extract_first()
        #     rate = each.xpath('.//*[contains(@class,"allstar")]/@class').extract_first()[7:8]
        #     comment_time = each.xpath('.//*[@class="comment-time "]/@title').extract_first()
        ...
        for each in comment_list:
            ...
            print('--------------------user_comment-----------------------')
            print('name           :', username)
            print('url_douban     :', user_id_douban)
            print('url_douban     :', url_douban)
            print('icon           :', icon)
            print('votes          :', votes)
            print('rate           :', rate)
            print('comment_time   :', comment_date)
    ...
    def parse_movie_review_douban(self, response):
        ...
        for each in review_list:
            ...
            print('url_douban     :', url_douban)
            print('icon           :', icon)
            print('review_href    :', review_href)
            print('user_id_douban :', user_id_douban)
    ...
    def parse_user_index_douban(self, response):
        ...
        comment_num = response.xpath('//*[@id="movie"]/h2/span/a[2]/text()').extract_first()[:-3]
        comment_url = response.xpath('//*[@id="movie"]/h2/span/a[2]/@href').extract_first()

        review_url = 'https://movie.douban.com/people/{id}/reviews'

        yield Request(
            url=comment_url + '?start=0&sort=time&rating=all&filter=all&mode=grid',
            meta={'user': user},
        ...
        # for page in range(10):
        #     yield Request(
        #         url=comment_url + '?start={num}&sort=time&rating=all&filter=all&mode=grid'.format(num=page * 15),
        #         meta={'user': user},
        #         callback=self.parse_user_comment_douban,
        #         headers=self.header,
        #         cookies=self.cookies,
    ...
    def parse_user_comment_douban(self, response):
        ...
        for each in comment_list:
            ...
            rate = each.xpath('.//*[contains(@class,"rating")]/@class').extract_first()[6]
    ...
    def parse_review_detail_douban(self, response):
        ...
        rate = response.xpath('//*[contains(@class,"allstar")]/@class').extract_first()[7]
        content = response.xpath('//*[@id="link-report"]/div[1]').extract_first()
        votes = response.xpath('//*[contains(@data-ad-ext,"有用")]/@data-ad-ext').extract_first().split(' ')[0][3:]
